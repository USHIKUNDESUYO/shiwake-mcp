...
総勘定元帳から仕訳を受け取り、14個のルールを当てて、先に人間が目を通すべき順番に並べ替えて返します。監査の現場でいう仕訳テスト（Journal Entry Testing）を、AIエージェントから呼べる形にしたものです。
...
102件のテストが走ります。MCP サーバーのテストは、子プロセスとして起こして実際に JSON-RPC を投げる経路で書いています。
